defmodule DAUWeb.IncomingMessageControllerTest do
  ...
  describe "index" do
    test "lists all incoming_messages", %{conn: conn} do
      conn = get(conn, ~p"/api/incoming_messages")
  ...
  describe "create incoming_message" do
    test "renders incoming_message when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/api/incoming_messages", incoming_message: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, ~p"/api/incoming_messages/#{id}")
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/api/incoming_messages", incoming_message: @invalid_attrs)
  ...
  describe "update incoming_message" do
    ...
    test "renders incoming_message when data is valid", %{conn: conn, incoming_message: %IncomingMessage{id: id} = incoming_message} do
      conn = put(conn, ~p"/api/incoming_messages/#{incoming_message}", incoming_message: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, incoming_message: incoming_message} do
      conn = put(conn, ~p"/api/incoming_messages/#{incoming_message}", incoming_message: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete incoming_message" do
    ...
    test "deletes chosen incoming_message", %{conn: conn, incoming_message: incoming_message} do
      conn = delete(conn, ~p"/api/incoming_messages/#{incoming_message}")
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, ~p"/api/incoming_messages/#{incoming_message}")
      end
    end
  end

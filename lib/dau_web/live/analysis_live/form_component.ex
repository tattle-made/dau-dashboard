defmodule DAUWeb.AnalysisLive.FormComponent do
  ...
  def update(%{analysis: analysis} = assigns, socket) do
    changeset = Canon.change_analysis(analysis)
  ...
  def handle_event("validate", %{"analysis" => analysis_params}, socket) do
    changeset =
      ...
      |> Canon.change_analysis(analysis_params)
  ...
  defp save_analysis(socket, :edit, analysis_params) do
    case Canon.update_analysis(socket.assigns.analysis, analysis_params) do
      {:ok, analysis} ->
  ...
  defp save_analysis(socket, :new, analysis_params) do
    case Canon.create_analysis(analysis_params) do
      {:ok, analysis} ->
        notify_parent({:saved, analysis})

...
defmodule DAU.UserMessage.Analytics.DeliveryReport do
  ...
  Build a structured delivery report from parameters sent by the bsp
  """
  def build_event(params) do
    try do
      params = params["statuses"]
      params = params |> hd
      event_type = Map.get(params, "status", nil)
      id = Map.get(params, "id", "ERROR-ERROR")
      ...
      {:ok, %__MODULE__{outbox_id: id, e_id: id, event_type: event_type_atom}}
    rescue
      _ -> {:error, "Badly Formatted Delivery Report from BSP"}
    end
  end
